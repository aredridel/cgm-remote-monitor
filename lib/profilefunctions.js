...
function init (profileData) {
  ...
  profile.updateTreatments = function updateTreatments (profiletreatments, tempbasaltreatments, combobolustreatments) {
    ...
    profile.profiletreatments_hash = createHash('sha1').update(JSON.stringify(profile.profiletreatments)).digest('hex');
    profile.tempbasaltreatments_hash = createHash('sha1').update(JSON.stringify(profile.tempbasaltreatments)).digest('hex');
    profile.combobolustreatments_hash = createHash('sha1').update(JSON.stringify(profile.combobolustreatments)).digest('hex');
  };
  ...
  profile.activeProfileTreatmentToTime = function activeProfileTreatmentToTime (time) {
    ...
    var treatment = null;
    if (profile.hasData()) {
      for (const t of profile.profiletreatments) {
  ...
  profile.tempBasalTreatment = function tempBasalTreatment (time) {
    ...
    var last = profile.tempbasaltreatments.length - 1;
  ...
  profile.comboBolusTreatment = function comboBolusTreatment (time) {
    var treatment = null;
    for (const t of profile.combobolustreatments) {
      var duration = times.mins(t.duration || 0).msecs;
  ...
  profile.listBasalProfiles = function listBasalProfiles () {
    ...
    if (profile.hasData()) {
      ...
      for (const key of Object.keys(profile.data[0].store)) {
