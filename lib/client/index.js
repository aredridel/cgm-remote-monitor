...
import language_ from '../language';
import sandbox_ from '../sandbox';
import profile_ from '../profilefunctions';
import units_ from '../units';
import levels from '../levels';
import times from '../times';
import receiveDData from './receiveddata.js';

var language = language_();
var sandbox = sandbox_();
...
new Loading({
  target: document.getElementById('loadingMessageText')
})
import browserSettings from './browser-settings.js';
import hashauth_ from '../hashauth';

client.hashauth = hashauth_.init(client, $);
...
import plugins_ from '../plugins/';
import utils_ from '../utils';
import bus_ from '../bus';
import notifications_ from '../notifications';
import boluscalc_ from './boluscalc.js';
import careportal_ from './careportal.js';
import renderer_ from './renderer.js';
import chart_ from './chart.js';
client.load = function load (serverSettings, callback) {
  ...
  var opacity = { current: 1, DAY: 1, NIGHT: 0.5 };
  var clientAlarms = {};
  var alarmInProgress = false;
  var alarmMessage;
  var currentNotify;
  var currentAnnouncement;
  var alarmSound = 'alarm.mp3';
  var urgentAlarmSound = 'alarm2.mp3';
  var previousNotifyTimestamp;
  ...
  client.ctx = {
    data: {}
    , bus: bus_(client.settings, client.ctx)
    , settings: client.settings
    , notifications: notifications_(client.settings, client.ctx)
  ...
  client.renderer = renderer_(client, d3, $);
  ...
  client.careportal = careportal_(client, $);
  client.boluscalc = boluscalc_(client, $);
  ...
  function brushed () {
    ...
    function updateCurrentSGV (entry) {
      ...
      var isCurrent = 'current' === client.timeago.checkStatus(client.sbx);
  ...
  function prepareEntries () {
    ...
    for (const point of client.entries) {
      if (point.mgdl < 39) {
  ...
  function dataUpdate (received) {
    ...
    if (!isInitialData) {
      isInitialData = true;
      chart = client.chart = chart_(client, d3, $);
      brushed();
      chart.update(true);
    } else if (!inRetroMode()) {
      chart.update(false);
